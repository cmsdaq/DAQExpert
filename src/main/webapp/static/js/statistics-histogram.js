...
function buildChart(containerId, title, step, factor, unit, max, fullData, key) {
	...
	if(dataNormalized.length > 0){  
		...
		$('#'+containerId).html("No data available in selected period to generate '" +title+ "' histogram");
	}
...
function buildHistograms(data) {
			buildChart('container-histogram-stable-beams',
					'Stable beams histogram', 2, 60 * 60, 'h', 50, data, 'stablebeamshistogram');
			buildChart('container-histogram-nrwe',
					'No rate when expected histogram', 5, 1, 'sec', 500, data, 'nrwehistogram');
			buildChart('container-histogram-run-ongoing',
					'Run ongoing duration histogram', 5, 60, 'min', 200, data, 'runongoinghistogram');
